...
const Header = () => {
  ...
  return <>
      ...
      <div className="bg-zinc-950/95 text-cream-dark py-2 px-4 hidden lg:block border-b border-zinc-800 backdrop-blur-xl">
        <div className="container mx-auto text-xs font-mono flex-row flex items-center justify-between gap-px">
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-cream-dark">Contact@JBLinxStudio.com</span>
            </div>
            <div className="flex items-center space-x-2">
              <Clock className="w-3 h-3 text-glow-amber" />
              <span className="text-cream-dark">Mon-Thurs: 11:00-17:00</span>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <MapPin className="w-3 h-3 text-glow-amber" />
            <span className="text-cream-dark">South Africa, EC</span>
          </div>
        </div>
      </div>
      ...
      <header className="sticky top-0 z-50 bg-zinc-900/95 backdrop-blur-2xl shadow-2xl border-b border-zinc-800">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-18">
            ...
            <Link to="/" className="flex items-center group">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-xl font-black text-cream font-mono">Linx</div>
            ...
            <nav className="hidden lg:flex items-center space-x-8">
              ...
              {navigationItems.map((item, index) => <NavigationItem key={index} name={item.name.toUpperCase()} href={item.href} type={item.type} categories={item.categories} />)}
            ...
            <button className="lg:hidden p-3 bg-zinc-800/50 hover:bg-zinc-700/50 transition-all duration-300 border border-zinc-700 backdrop-blur-sm" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {isMenuOpen && <div className="lg:hidden absolute top-full left-0 right-0 bg-zinc-900/98 backdrop-blur-2xl shadow-2xl border-t border-zinc-800 z-50 animate-in slide-in-from-top-4 duration-300">
              <div className="px-4 py-6 space-y-4 max-h-96 overflow-y-auto">
                <Link to="/" className="block py-3 text-cream-dark hover:text-glow-orange transition-colors font-bold text-sm font-mono tracking-wider border-b border-zinc-800" onClick={() => setIsMenuOpen(false)}>
                  HOME
                </Link>
                
                {navigationItems.map((item, index) => <div key={index}>
                    {item.type === 'link' ? <Link to={item.href} className="block py-3 text-cream-dark hover:text-glow-orange transition-colors font-bold text-sm font-mono tracking-wider border-b border-zinc-800" onClick={() => setIsMenuOpen(false)}>
                        {item.name.toUpperCase()}
                      </Link> : <div className="border-b border-zinc-800">
                        <div className="py-3 text-glow-orange font-bold text-sm font-mono tracking-wider">{item.name.toUpperCase()}</div>
                        {item.categories?.map((category, catIndex) => <div key={catIndex} className="ml-4 mb-4">
                            <h4 className="font-bold text-glow-emerald mb-2 text-xs font-mono">{category.title.toUpperCase()}</h4>
                            <div className="space-y-2">
                              {category.items.map((subItem, subIndex) => <Link key={subIndex} to={subItem.href} className="block py-2 px-3 text-cream-dark hover:text-glow-orange hover:bg-zinc-800/50 transition-all duration-300 text-xs border border-zinc-800/50 hover:border-orange-400/30 backdrop-blur-sm" onClick={() => setIsMenuOpen(false)}>
                ...
                <Link to="/blog" className="block py-3 text-cream-dark hover:text-glow-orange transition-colors font-bold text-sm font-mono tracking-wider border-b border-zinc-800" onClick={() => setIsMenuOpen(false)}>
                  BLOG
                </Link>
