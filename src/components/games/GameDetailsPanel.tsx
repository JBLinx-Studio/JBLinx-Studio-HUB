...
const GameDetailsPanel: React.FC<GameDetailsPanelProps> = ({ game }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'LIVE': return 'bg-green-500 text-white shadow-green-500/50';
      case 'BETA': return 'bg-yellow-500 text-black shadow-yellow-500/50';
      case 'EARLY_ACCESS': return 'bg-orange-500 text-white shadow-orange-500/50';
  ...
  return (
    <div className="h-full bg-gradient-to-b from-slate-900 via-slate-800/95 to-slate-900 flex flex-col backdrop-blur-sm">
      {/* Enhanced Header */}
      <div className="p-6 border-b border-slate-700/50 bg-slate-900/90 backdrop-blur-sm">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                {game.platforms.slice(0, 4).map((platform, index) => (
                  <div key={index} className="p-2 bg-slate-800/50 border border-slate-600/50 text-slate-400 hover:text-white transition-colors" title={platform}>
              ...
              <div className="flex flex-wrap gap-1">
                {game.genres.slice(0, 3).map((genre, index) => (
                  <span key={index} className="bg-cyan-500/20 border border-cyan-400/30 text-cyan-400 px-2 py-1 text-xs font-bold">
                    {genre}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center justify-end space-x-2 mb-3">
              <button
                ...
                className={`group p-3 transition-all duration-300 ${
                  isWishlisted ? 'bg-red-500 text-white shadow-red-500/30' : 'bg-slate-800/50 text-slate-400 hover:bg-red-500 hover:text-white'
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { icon: Star, label: 'Rating', value: `${game.rating}★`, color: 'text-yellow-400', bg: 'bg-yellow-500/20' },
            { icon: Users, label: 'Players', value: game.playerCount, color: 'text-green-400', bg: 'bg-green-500/20' },
            { icon: Download, label: 'Reviews', value: `${(game.reviewCount / 1000).toFixed(1)}K`, color: 'text-blue-400', bg: 'bg-blue-500/20' },
            { icon: Trophy, label: 'Peak', value: `${(game.stats.peakPlayers / 1000).toFixed(0)}K`, color: 'text-purple-400', bg: 'bg-purple-500/20' }
          ].map((stat, index) => {
            const IconComponent = stat.icon;
            return (
              <div key={index} className={`${stat.bg} border border-slate-600/30 p-3 text-center hover:scale-105 transition-transform duration-300 backdrop-blur-sm`}>
              ...
              <div className="flex space-x-3 overflow-x-auto pb-2">
                {[game.images.hero, ...game.images.gallery].slice(0, 6).map((image, index) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-20 h-20 border-2 transition-all duration-300 hover:scale-110 ${
                      index === activeImage ? 'border-purple-400 shadow-purple-400/50' : 'border-slate-600/50 hover:border-slate-500'
                    } shadow-lg`}
                  >
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {game.features.slice(0, 6).map((feature, index) => (
                    <div key={index} className="flex items-center space-x-3 p-2 bg-slate-800/30 border border-slate-700/50 hover:border-cyan-400/30 transition-colors">
